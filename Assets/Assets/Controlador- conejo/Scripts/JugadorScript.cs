...
public class JugadorScript : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.tag == "ZonaSegura" && agachadoCollider.enabled == true)
        {
            jugador.layer = LayerMask.NameToLayer("ZonaSegura");
        }

        if(collision.gameObject.tag == "Checkpoint") {
            TriggerCheckpoint(collision.gameObject);
        }
    }

    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "ZonaSegura" && pieCollider.enabled == true)
        {
            jugador.layer = LayerMask.NameToLayer("jugador");
        }
    }
    ...
    private void TriggerCheckpoint(GameObject go){
